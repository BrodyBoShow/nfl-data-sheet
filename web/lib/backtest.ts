...
export interface BacktestSummary {
  ...
  buckets: Record<
    string,
    {
      ...
      margin_sd: number;
      spread: EdgeValidation;
      total: EdgeValidation;
    }
  >;
  ...
  model: {
    ...
    n_games: number;
    alpha: Coefficient;
    beta_off: Coefficient;
    beta_def: Coefficient;
    gamma: Coefficient;
  };
  report: { script: string; generated_utc: string }; // from the report's own "Generated by" line
  win_rates: WinRateBucket[]; // the report's two tables by |edge| bucket
...
export interface WinRateBucket {
  ...
  bucket: string; // "0–1", …, "4+" (points of |edge|)
  decided: number;
  wins: number;
  rate: number;
}
